...
generator_test_padchest = img_generator.flow_from_dataframe(dataframe = annotations, 
    x_col='ImagePath',
    y_col='Chest_drain',
    target_size=(512, 512),
    classes=None,
...
filename = "Predictions/MT_preds_TD.csv"
...
all_dict["Val_data"].append('PadChest, TD')

for i in range(len(json)):
    model = load_model(json[i], h5[i])
    pred = make_predictions(model, generator_test_padchest)[1]
